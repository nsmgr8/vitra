...
def confirm(text):
    return int(u_vim.eval('confirm("{0}", "&Yes\n&No", 2)'.format(text))) != 2
...
class Window(object):
    ...
    def buffer_name(self):
        return u'{0}:\ {1}'.format(self.prefix, self.name)
        ...
        for values in options.values():
            for i, a in enumerate(values):
                try:
                    ...
                    u_vim.command(u'syn match {0} /\<{1}\>/'.format(hi,
                                a.replace(u'/', u'\/')))
...
class Wiki(object):
    ...
    def save(self, comment):
        ...
        try:
            ...
            u_vim.command('echoerr "Error: {0}"'.format(e.faultString))
        except Exception as e:
            print_error(e)
        return False
...
class Ticket(object):
    ...
    def _delete_vim_commands(self):
        for t in self.options.get('type', []):
            ...
            u_vim.command('delc TTCreate{0}'.format(name))
        delcommand = """
            if exists(':TT{0}{1}') == 2
                delc TT{0}{1}
            endif
    ...
    def _generate_vim_commands(self):
        for t in self.options['type']:
            ...
            u_vim.command('com! -nargs=+ {0} {1}'.format(name, meth))
...
class Trac(object):
    ...
    def server_view(self):
        server_list = u_vim.eval('tracServerList')
        default = '{0}: '.format(u_vim.eval('tracDefaultServer'))
